<template>
	...
	<div class="list-item fst-point-won" v-for="item in fstPointWon">
...
<script>
	export default {
		...
		ready() {
			...
			let api = `/atp/stats/1st-serve-points-won/${currentYear}/all`;			
			
			this.$http.get(api)
				.then( (res) => {
					this.fstPointWon = res.data.detail;
					this.title = currentYear + '一发赢球率排名';
